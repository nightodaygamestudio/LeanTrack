...
function Onboarding({ initial, onComplete, t, units }){
  ...
  // derived display states for units
  ...
  return (<div className="screen">
    ...
    <button className="btn primary" disabled={!ready} style={{marginTop:20}}
      onClick={()=>{
        ...
        setRaw(STORAGE.welcomed, "0"); // Splash ab nächstem Start
...
function Today({ state, setState, profile, goals, t, units }){
  ...
  const waterTarget = units.volume==='ml' ? Number(goals?.dailyWaterMl)||0
                    : Math.round(mlToFlOz(Number(goals?.dailyWaterMl)||0));
  const proteinTarget = Number(goals?.dailyProteinG)||0;

  const waterNow = units.volume==='ml' ? (Number(water)||0)
  ...
  // Activity kcal estimate uses internal SI
    ...
    <div className="card-group">
      ...
      <div className="card-input">
        ...
        <input type="number" placeholder={units.volume==='ml'?'1500':'50'} value={
          (()=> units.volume==='ml' ? (water ?? '') : (water ? water : ''))()
        } onChange={(e)=> {
      ...
      <div className="card-input">
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
          ...
          <input type="number" placeholder={units.distance==='km'?t('km'):t('mi')} value={(()=>{
            if(units.distance==='km') return distanceKm ?? '';
            const km = Number(distanceKm)||0; return km? String(Math.round(kmToMi(km)*100)/100) : '';
          })()} onChange={(e)=>{
            const num=Number(e.target.value)||0;
            if(units.distance==='km') setState(s=>({...s, distanceKm:e.target.value}));
            else setState(s=>({...s, distanceKm:String(Math.round(miToKm(num)*100)/100)}));
          }}/>
...
function MenuSheet({ open, onClose, t, lang, setLang, units, setUnits }){
  ...
  const [mode, setMode] = useState('daily'); // daily|weekdays
...
function App(){
  ...
  useEffect(()=>{ const f=getRaw(STORAGE.welcomed)||"0"; if(f==="0") setRaw(STORAGE.welcomed,"1"); else if(f==="1"){ setShowSplash(true); setRaw(STORAGE.welcomed,"2"); const to=setTimeout(()=> setShowSplash(false), 1400); return ()=> clearTimeout(to);} }, []);
